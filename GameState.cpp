...
GameState::GameState(std::string& load_data, bool json)
{
    ...
    prog_stars[PROG_LOCK_GROUPS] = 3000;
...
void GameState::reset_levels()
{
    ...
    if (game_mode == 4)
    {
        for (unsigned i = 0; i < neg_server_levels.size(); i++)
        {
...
void GameState::robot_thread(int thread_index)
{
    ...
    while (true)
    {
                ...
                for (unsigned g = 0; g < (GLBAL_LEVEL_SETS + 1); g++)
                {
                    ...
                    for (unsigned s = 0; s < level_progress[game_mode][g].size(); s++)
                    {
                        ...
                        for (unsigned i = 0; i < level_progress[game_mode][g][s].level_status.size(); i++)
                        {
                            ...
                            if (!p.done && !p.robot_done)
                            {
                                for (int r = 0; r < int(p.stats/100 + 1); r++)
        ...
        while (true)
        {
            static const XYSet emptyFilter{};
            int rep = advance_grid(grid, rules[game_mode], NULL, emptyFilter, emptyFilter, true);
...
static int advance_grid(Grid* grid, std::list<GridRule>& rules, GridRegion* inspected_region, const XYSet& filter_pos_and, const XYSet& filter_pos_not, bool skip_hide)
{
    grid->add_base_regions();
    // for (GridRegion& r : grid->regions)
    //     r.stale = true;
    
    GridRegion* new_region = grid->add_one_new_region(inspected_region, filter_pos_and, filter_pos_not);
...
void GameState::update_constructed_rule()
{
    ...
    const bool valid_structure = constructed_rule.has_valid_structure();
    if (!valid_structure)
        constructed_rule_is_logical = GridRule::ILLOGICAL;
    ...
    const int input_count = valid_structure ?
        constructed_rule.region_count - constructed_rule.if_reg_count : 0;
    for(int i = input_count; i < 4; i++)
...
bool GameState::render_lock(int lock_type, XYPos pos, XYPos size)
{
    ...
    render_box(pos, size, button_size/4, 4);
...
{
        auto rule_gen_colour_for_slot = [&](int slot) -> unsigned {
            if (right_panel_mode != RIGHT_MENU_RULE_GEN || !rule.has_valid_structure())
                return 0;
            GridRegion* region = rule_gen_region[rule.input_index_for_slot(slot)];
            return region ? region->colour : 0;
        };
...
void GameState::render(bool saving)
{
    ...
    if (right_panel_mode == RIGHT_MENU_RULE_GEN || right_panel_mode == RIGHT_MENU_RULE_INSPECT)
    {
        ...
        if (hover_rulemaker_region_base_index >= 0 && rule_cause.rule->has_valid_structure())
        {
            hover_rulemaker_region_base = true;
            const int input_index = rule_cause.rule->input_index_for_slot(hover_rulemaker_region_base_index);
            mouse_hover_region = rule_cause.regions[input_index];
        }
        ...
        if ((mouse - (right_panel_offset + XYPos(0, button_size * 2))).inside(XYPos(button_size*4, button_size * 6)))
        {
            ...
            if (hover_rulemaker_bits >= 1)
            {
                ...
                if (!rule_cause.rule->has_valid_structure())
                    hover_squares_highlight.clear();
                for (int i = 0; i < rule_cause.rule->region_count; i++)
                {
                    if (!rule_cause.rule->has_valid_structure())
                        break;
                    const int cause_index = rule_cause.rule->input_index_for_slot(i);
                    const bool use_neg = i < rule_cause.rule->if_reg_count * 2 && !(i & 1);
    ...
    if (display_scores)
    {
        ...
        for (unsigned score_index = 0; score_index < unsigned(display_table_row_count); score_index++)
        {
            ...
            if (score_index + scores_list_offset >= score_tables[game_mode][grp_index].size())
                break;
            PlayerScore& s = score_tables[game_mode][grp_index][score_index + scores_list_offset];
            if (s.hidden)
                continue;
    ...
    else if (display_levels)
    {
        ...
        for (int level_index = 0; level_index < display_table_row_count; level_index++)
        {
            ...
            if (level_index + levels_list_offset >= (int)levels_list.size())
                break;
            unsigned index = levels_list[level_index + levels_list_offset];
...
//            render_box(XYPos(dst_rect.x, dst_rect.y), XYPos(dst_rect.w, dst_rect.h), cell_width / 4, 2);
        ...
        {
            SDL_Rect src_rect = {2816, 1344, 192, 192};
            SDL_Rect dst_rect = {list_pos.x + 1 * cell_width, list_pos.y, cell_width, cell_width};
            SDL_RenderCopy(sdl_renderer, sdl_texture, &src_rect, &dst_rect);
...
//                render_box(XYPos(dst_rect.x, dst_rect.y), XYPos(dst_rect.w, dst_rect.h), cell_width / 4, 2);
                ...
                if (display_rules_click && ((display_rules_click_pos - XYPos(dst_rect.x, dst_rect.y)).inside(XYPos(dst_rect.w, dst_rect.h))))
                {
                    if (display_rules_cpu)
                    {
                        display_rules_cpu = false;
                        display_rules_level = false;
                    }
                    else if (display_rules_level)
                        display_rules_cpu = true;
                    else
                        display_rules_level = true;
                }
            }
            {
...
//                render_box(XYPos(dst_rect.x, dst_rect.y), XYPos(dst_rect.w, dst_rect.h), cell_width / 4, 2);
        ...
        {
            SDL_Rect src_rect = {704 + 3 * 192, 2144, 96, 96};
            SDL_Rect dst_rect = {list_pos.x + 8 * cell_width, list_pos.y, cell_width / 2, cell_width / 2};
            SDL_RenderCopy(sdl_renderer, sdl_texture, &src_rect, &dst_rect);
            add_tooltip(dst_rect, "Close");
            if (display_rules_click && ((display_rules_click_pos - XYPos(dst_rect.x, dst_rect.y)).inside(XYPos(dst_rect.w, dst_rect.h))))
            {
        ...
        struct RuleDiplaySort
        {
            ...
            bool operator() (RuleDiplay a_,RuleDiplay b_)
            {
                ...
                if (col == 1)
                    return (a.rule->group < b.rule->group);
                if (col == 2)
                {
                ...
                if (cpu_debug && (col == 7))
                {
                    ...
                    if (!a.rule->clear_count && a.rule->cpu_time)
                        return false;
                    if (!b.rule->clear_count && b.rule->cpu_time)
                        return true;

                    double fa = (double)a.rule->cpu_time / (double)a.rule->clear_count;
                    double fb = (double)b.rule->cpu_time / (double)b.rule->clear_count;
                    if (!a.rule->cpu_time)
                        fa = 0;
                    if (!b.rule->cpu_time)
                        fb = 0;
        ...
        std::stable_sort (rules_list.begin(), rules_list.end(), RuleDiplaySort(display_rules_sort_col_2nd, display_rules_sort_dir_2nd, display_rules_level, grid, display_rules_cpu));
        std::stable_sort (rules_list.begin(), rules_list.end(), RuleDiplaySort(display_rules_sort_col, display_rules_sort_dir, display_rules_level, grid, display_rules_cpu));
        ...
        if (display_rules_center_current)
            for (unsigned i = 0; i < rules_list.size(); i++)
                if (rules_list[i].rule == inspected_rule.rule)
                {
...
//                                        to--;
        ...
        {
            SDL_Rect src_rect = {1664, 1344, 64, 64};
            SDL_Rect dst_rect = {list_pos.x + cell_width * 8, list_pos.y + cell_width * 1, cell_width/2, cell_width/2};
            SDL_RenderCopy(sdl_renderer, sdl_texture, &src_rect, &dst_rect);
            if (display_rules_click && ((display_rules_click_pos - XYPos(dst_rect.x, dst_rect.y)).inside(XYPos(dst_rect.w, dst_rect.h))))
                rules_list_offset--;
        }
        {
            SDL_Rect src_rect = {1664, 1408, 64, 64};
            SDL_Rect dst_rect = {list_pos.x + cell_width * 8, list_pos.y + cell_width * 8, cell_width/2, cell_width/2};
            SDL_RenderCopy(sdl_renderer, sdl_texture, &src_rect, &dst_rect);
            if (display_rules_click && ((display_rules_click_pos - XYPos(dst_rect.x, dst_rect.y)).inside(XYPos(dst_rect.w, dst_rect.h))))
                rules_list_offset++;
        }
    ...
    else
    {
        ...
        }
        {
            ...
            for (GridRegion& region : grid->regions)
            {
                if ((ctrl_held || region.matches_filters(filter_pos_and, filter_pos_not)) &&
                    (region.vis_level == vis_level))
                {
...
//                        SDL_SetTextureColorMod(sdl_texture, contrast, contrast, contrast);
    ...
    if (render_lock(PROG_LOCK_SPEED, XYPos(left_panel_offset.x + 0 * button_size, left_panel_offset.y + button_size * 1), XYPos(button_size * 3, button_size)))
    {
        ...
        SDL_RenderCopy(sdl_renderer, sdl_texture, &src_rect, &dst_rect);
        bool hover = add_tooltip(dst_rect, "Speed", false);
        src_rect = {hover ? 1344 : 1280, 1920, 64, 192};
    ...
    {
        int h = 0;
        if (max_stars >= prog_stars[PROG_LOCK_TABLE_RULES]) h = 1;
        if (max_stars >= prog_stars[PROG_LOCK_TABLE_LEVELS]) h = 2;
        if (max_stars >= prog_stars[PROG_LOCK_TABLE_SCORES]) h = 3;
        if (h)
            render_box(left_panel_offset + XYPos(button_size * 0, button_size * 2), XYPos(button_size * 2, button_size * h), button_size/4, 4);


        if (render_lock(PROG_LOCK_TABLE_RULES, XYPos(left_panel_offset.x + 0 * button_size, left_panel_offset.y + 2 * button_size), XYPos(button_size * 2, button_size)))
        {
            ...
            SDL_RenderCopy(sdl_renderer, sdl_texture, &src_rect, &dst_rect);
            render_number_string(digits, left_panel_offset + XYPos(button_size * 1 + button_size / 8, button_size * 2 + button_size / 20), XYPos(button_size * 3 / 4, button_size * 8 / 20), XYPos(1,0));
            render_number(vis_rule_count, left_panel_offset + XYPos(button_size * 1 + button_size / 8, button_size * 2 + button_size / 2 + button_size / 20), XYPos(button_size * 3 / 4, button_size * 8 / 20), XYPos(1,0));
            dst_rect.w *= 2;
            add_tooltip(dst_rect, "Rules", !display_rules);
        }
        if (render_lock(PROG_LOCK_TABLE_LEVELS, XYPos(left_panel_offset.x + 0 * button_size, left_panel_offset.y + 3 * button_size), XYPos(button_size * 2, button_size)))
        {
        ...
        if (render_lock(PROG_LOCK_TABLE_SCORES, XYPos(left_panel_offset.x + 0 * button_size, left_panel_offset.y + 4 * button_size), XYPos(button_size * 2, button_size)))
        {
            ...
            SDL_RenderCopy(sdl_renderer, sdl_texture, &src_rect, &dst_rect);
            render_number(count, left_panel_offset + XYPos(button_size * 1 + button_size / 8, button_size * 4 + button_size / 4), XYPos(button_size * 3 / 4, button_size / 2), XYPos(1,0));
            dst_rect.w *= 2;
    ...
    if (render_lock(PROG_LOCK_VISIBILITY, XYPos(left_panel_offset.x + 3 * button_size, left_panel_offset.y + 2 * button_size), XYPos(2 * button_size, 3 * button_size)))
    {
        ...
        int region_vis_counts[3][2] = {};
        for (GridRegion& region : grid->regions)
        {
            region_vis_counts[int(region.vis_level)][!region.matches_filters(filter_pos_and, filter_pos_not)]++;
        }
        {
            ...
            if (region_vis_counts[0][1] == 0)
            {
                render_number(region_vis_counts[0][0], left_panel_offset + XYPos(button_size * 3 + button_size / 8, button_size * 2 + button_size / 4), XYPos(button_size * 3 / 4, button_size / 2), XYPos(1, 0));
            }
            else
            {
                render_number(region_vis_counts[0][0], left_panel_offset + XYPos(button_size * 3 + button_size / 8, button_size * 2 + button_size * 3 / 16), XYPos(button_size * 3 / 4, button_size / 4), XYPos(1, 0));
                render_number(region_vis_counts[0][1], left_panel_offset + XYPos(button_size * 3 + button_size / 8, button_size * 2 + button_size * 9 / 16), XYPos(button_size * 3 / 4, button_size / 4), XYPos(1, 0));
            }
            SDL_RenderCopy(sdl_renderer, sdl_texture, &src_rect, &dst_rect);
        ...
        }
        {
            ...
            if (region_vis_counts[1][1] == 0)
            {
                render_number(region_vis_counts[1][0], left_panel_offset + XYPos(button_size * 3 + button_size / 8, button_size * 3 + button_size / 4), XYPos(button_size * 3 / 4, button_size / 2), XYPos(1, 0));
            }
            else
            {
                render_number(region_vis_counts[1][0], left_panel_offset + XYPos(button_size * 3 + button_size / 8, button_size * 3 + button_size * 3 / 16), XYPos(button_size * 3 / 4, button_size / 4), XYPos(1, 0));
                render_number(region_vis_counts[1][1], left_panel_offset + XYPos(button_size * 3 + button_size / 8, button_size * 3 + button_size * 9 / 16), XYPos(button_size * 3 / 4, button_size / 4), XYPos(1, 0));
            }
            SDL_RenderCopy(sdl_renderer, sdl_texture, &src_rect, &dst_rect);
        ...
        }
        {
            ...
            if (region_vis_counts[2][1] == 0)
            {
                render_number(region_vis_counts[2][0], left_panel_offset + XYPos(button_size * 3 + button_size / 8, button_size * 4 + button_size / 4), XYPos(button_size * 3 / 4, button_size / 2), XYPos(1, 0));
            }
            else
            {
                render_number(region_vis_counts[2][0], left_panel_offset + XYPos(button_size * 3 + button_size / 8, button_size * 4 + button_size * 3 / 16), XYPos(button_size * 3 / 4, button_size / 4), XYPos(1, 0));
                render_number(region_vis_counts[2][1], left_panel_offset + XYPos(button_size * 3 + button_size / 8, button_size * 4 + button_size * 9 / 16), XYPos(button_size * 3 / 4, button_size / 4), XYPos(1, 0));
            }
            SDL_RenderCopy(sdl_renderer, sdl_texture, &src_rect, &dst_rect);
        ...
        for (unsigned i = 0; i < level_progress[game_mode][current_level_group_index].size(); i++)
        {
            ...
            if (c)
            {
                ...
                if (run_robots)
                {
                    bool todo = false;
                    bool active = false;
                    for (LevelStatus& l : level_progress[game_mode][current_level_group_index][i].level_status)
                    {
                        if (!l.done)
                        {
                            if (l.robot_done == 0)
                    ...
                    SDL_Rect src_rect = {896, 1344, 192, 192};
                    if (todo)
                        src_rect = {3008, 1344, 192, 192};
                    if (active)
                        src_rect = {704, 2336, 192, 192};
                    dst_rect = {pos.x + button_size * 2 / 3, pos.y + button_size * 2 / 3, button_size / 3, button_size / 3};
                    SDL_RenderCopy(sdl_renderer, sdl_texture, &src_rect, &dst_rect);
                }
            }
            else
    ...
    if (right_panel_mode == RIGHT_MENU_REGION)
    {
        ...
        render_button(XYPos(896, 1152), XYPos(right_panel_offset.x + button_size * 0, right_panel_offset.y + 5 * button_size), "Change Color");
    ...
    if (right_panel_mode == RIGHT_MENU_RULE_GEN || right_panel_mode == RIGHT_MENU_RULE_INSPECT)
    {
        ...
        if (right_panel_mode == RIGHT_MENU_RULE_GEN)
        {
            ...
            if (constructed_rule.region_count >= 1)
            {
                ...
                {
                    render_button(XYPos(1280, 1152), XYPos(right_panel_offset.x + button_size * 4, right_panel_offset.y + button_size), "Redundant", 1);
                }
                else if (constructed_rule.apply_region_bitmap)
                {
                    if (constructed_rule_is_logical == GridRule::ILLOGICAL)
                    {
                        if (render_button(XYPos(896, 576), XYPos(right_panel_offset.x + button_size * 4, right_panel_offset.y + button_size), "Illogical", 1))
                        {
                            ...
                            for (int i = 0; i < 5; i++)
                            {
                                ...
                                render_number_string(v, right_panel_offset + XYPos(int(-7.7 * button_size), (i + 1) * button_size), XYPos(button_size * 1.6, button_size * 0.8),XYPos(-1, 0));
                            }
        ...
        if (right_panel_mode == RIGHT_MENU_RULE_INSPECT)
        {
            ...
            if (max_stars >= prog_stars[PROG_LOCK_TABLE_SCORES])
                render_button(XYPos(2496, 384), XYPos( right_panel_offset.x + button_size * 2, right_panel_offset.y + button_size * 9), "Clear Rule Statistics");
            if (max_stars >= prog_stars[PROG_LOCK_LEVELS_AND_LOCKS])
                render_button(XYPos(1856, 1536), XYPos( right_panel_offset.x + button_size * 3, right_panel_offset.y + button_size * 6), selected_rules.size() == 1 ? "Copy Rule to Clipboard" : "Copy Rules to Clipboard");
            ...
            if (max_stars >= prog_stars[PROG_LOCK_GROUPS])
            {
                int group = (*selected_rules.begin())->group;
                for (GridRule* rule : selected_rules)
                    if (rule->group != group)
                        group = -1;

                render_box(XYPos(right_panel_offset.x + button_size * 1, right_panel_offset.y + button_size * 7), XYPos(button_size * 4, button_size * 2), button_size/4, 4);
                for (int i = 0; i < 8; i++)
                {
                    if (i == group)
            ...
            if (inspected_rule.rule->apply_region_type.type != RegionType::VISIBILITY && (max_stars >= prog_stars[PROG_LOCK_TABLE_RULES]))
            {
                ...
                render_number(grid->level_used_count[inspected_rule.rule], right_panel_offset + XYPos(button_size * 3 + button_size / 8, button_size * 10 + button_size / 4), XYPos(button_size * 3 / 4, button_size / 2), XYPos(1, 0));
    ...
    if (right_panel_mode == RIGHT_MENU_NONE)
    {
        ...
        if (render_lock(PROG_LOCK_ROBOTS, XYPos(right_panel_offset.x + 1 * button_size, right_panel_offset.y + button_size * 3), XYPos(button_size * 3, button_size * 3)))
        {
            ...
            {
                render_number(done_count, XYPos(right_panel_offset.x + 2 * button_size, right_panel_offset.y + button_size * 4.25), XYPos(button_size * 0.9, button_size/2), XYPos(1, 0));
                render_number(todo_count, XYPos(right_panel_offset.x + 2 * button_size, right_panel_offset.y + button_size * 5.25), XYPos(button_size * 0.9, button_size/2), XYPos(1, 0));
            }
            {
        ...
        if (render_lock(PROG_LOCK_PAINT, XYPos(right_panel_offset.x + 0 * button_size, right_panel_offset.y + button_size * 9), XYPos(button_size * 1, button_size * 1)))
        {
            render_button(XYPos(1856, 576), XYPos(right_panel_offset.x + button_size * 0, right_panel_offset.y + button_size * 9), "Paint", (mouse_mode == MOUSE_MODE_PAINT) ? 3 : 0);
            if (!overlay_texture_is_clean)
                render_button(XYPos(896, 1536), XYPos(right_panel_offset.x + button_size * 0, right_panel_offset.y + button_size * 10), "Reset Paint", 0);
            if (mouse_mode == MOUSE_MODE_PAINT)
            {
                ...
                render_box(right_panel_offset + XYPos(button_size * 4, button_size * 9), XYPos(button_size * 1, button_size * 3), button_size/4, 4);
                for (int i = 0; i < 3; i++)
                {
                    if (i == paint_brush_size)
                        render_box(right_panel_offset + XYPos(button_size * 4, button_size * 9 + i * button_size), XYPos(button_size, button_size), button_size/4, 10);
                    
                    int size = (i == 0) ? button_size / 4 :
                               (i == 1) ? button_size / 2 : button_size;
                    render_box(right_panel_offset + XYPos(4 * button_size + (button_size - size) / 2, button_size * 9 + i * button_size + (button_size - size) / 2), XYPos(size, size), size/4, 11);

                    SDL_Rect dst_rect = {right_panel_offset.x + 4 * button_size, right_panel_offset.y + button_size * 9 + i * button_size, button_size, button_size};
                    add_clickable_highlight(dst_rect);
    ...
    if (display_menu)
    {
        ...
        if (render_lock(PROG_LOCK_FAST_HINT1, XYPos(left_panel_offset.x + 11 * button_size, left_panel_offset.y + button_size * 8), XYPos(button_size, button_size)))
        {
        ...
        if (render_lock(PROG_LOCK_FAST_HINT2, XYPos(left_panel_offset.x + 12 * button_size, left_panel_offset.y + button_size * 8), XYPos(button_size, button_size)))
        {
...
void GameState::left_panel_click(XYPos pos, int clicks, int btn)
{
    ...
    {
        skip_level = (btn == 2 || ctrl_held || shift_held) ? -1 : 1;
        force_load_level = false;
    }
    ...
    if (prog_seen[PROG_LOCK_TABLE_RULES])
    {
    ...
    if (prog_seen[PROG_LOCK_TABLE_LEVELS])
    {
    ...
    if (prog_seen[PROG_LOCK_TABLE_SCORES])
    {
...
void GameState::right_panel_click(XYPos pos, int clicks, int btn)
{
    ...
    if (right_panel_mode == RIGHT_MENU_RULE_INSPECT)
    {
        ...
        if ((pos - XYPos(button_size * 3, button_size * 2)).inside(XYPos(button_size * 2, button_size)))
        {
            if (!inspected_rule.rule->has_valid_structure())
                return;
            reset_rule_gen_region();
            ...
            const int input_count = constructed_rule.region_count - constructed_rule.if_reg_count;
            for (int i = 0; i < input_count; i++)
                rule_gen_region[i] = inspected_rule.regions[i];
        ...
        if ((pos - XYPos(button_size * 2, button_size * 9)).inside(XYPos(button_size, button_size)))
        {
            for (GridRule* rule : selected_rules)
            {
                rule->used_count = 0;
                rule->clear_count = 0;
                rule->cpu_time = 0;
                grid->level_used_count[rule] = 0;
                grid->level_clear_count[rule] = 0;
            }
        }
        ...
        }
        {
            if (max_stars >= prog_stars[PROG_LOCK_GROUPS])
            {
                if ((pos - XYPos(button_size * 1, button_size * 7)).inside(XYPos(button_size * 4, button_size * 2)))
                {
                    XYPos p = ((pos - XYPos(button_size * 1, button_size * 7)) / button_size);
                    int sel = p.x + p.y * 4;
                    for (GridRule* rule : selected_rules)
                        rule->group = sel;
                }
    ...
    if (right_panel_mode == RIGHT_MENU_NONE)
    {
        ...
        if ((prog_stars[PROG_LOCK_PAINT] <= max_stars) && (pos - XYPos(button_size * 0, button_size * 9)).inside(XYPos(button_size, button_size)))
        {
            if(mouse_mode == MOUSE_MODE_PAINT)
                mouse_mode = MOUSE_MODE_NONE;
            else
        ...
        if ((prog_stars[PROG_LOCK_PAINT] <= max_stars) && (pos - XYPos(button_size * 0, button_size * 10)).inside(XYPos(button_size, button_size)))
        {
            if(!overlay_texture_is_clean)
                clear_overlay();
            return;
        }
        if (mouse_mode == MOUSE_MODE_PAINT && (pos - XYPos(button_size * 1, button_size * 9)).inside(XYPos(button_size * 3, button_size * 3)))
        {
        ...
        if (mouse_mode == MOUSE_MODE_PAINT && (pos - XYPos(button_size * 4, button_size * 9)).inside(XYPos(button_size * 1, button_size * 3)))
        {
            XYPos p = pos - XYPos(button_size * 4, button_size * 9);
            p /= button_size;
    ...
    if (right_panel_mode == RIGHT_MENU_RULE_GEN || right_panel_mode == RIGHT_MENU_RULE_INSPECT)
    {
        ...
        if (region_index >= 0)
        {
            if (!rule_cause.rule->has_valid_structure())
                return;
            const int input_index = rule_cause.rule->input_index_for_slot(region_index);
            if (right_panel_mode == RIGHT_MENU_RULE_GEN)
            {
                if (ctrl_held)
                {
                ...
                if (btn == 2)
                {
                    ...
                    for (int i = input_index; i < new_count; i++)
                        rule_gen_region[i] = rule_gen_region[i+1];
                ...
                if (region_type != constructed_rule.region_type[region_index])
                {
                    ...
                    rule_gen_region[input_index] = NULL;
                    update_constructed_rule();
                    return;
                }
            }
            else if (rule_cause.regions[input_index])
            {
                right_panel_mode = RIGHT_MENU_REGION;
                inspected_region = rule_cause.regions[input_index];
            }
            return;
        }
    }
                    ...
                    else if (region_type.type != RegionType::VISIBILITY && region_type.type != RegionType::NONE)
                    {
                        ...
                        if (if_then_selected == 1)
                        {
                            ...
                            if (!(constructed_rule.neg_apply_region_bitmap & (1 << hover_rulemaker_bits)))
                                ...
                                constructed_rule.neg_apply_region_bitmap ^= 1 << hover_rulemaker_bits;
                            if (!constructed_rule.neg_apply_region_bitmap)
                                constructed_rule.apply_if_region_type = RegionType();
                        }
